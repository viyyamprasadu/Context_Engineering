## 1. simplify system_prompt 
- pass arguments to the sys prompt function

## 2. simplify user_prompt
- give input as arguments
- dynamic user input data

## 3. Modify the system prompt suchway that the output should be in standard format

## 4. Add format specifiers such JSON,etc in the main

## 5. Change the way of the getting the output as txt,md,etc.. with standard naming of the files(instead of fixed file)

## 6. Design one image/video generation app using open source image/video generation models.

## 6. I want to build a new trading system which can excute trades on its own based on the strategy created for the specific instrument via specific broker. Create new model to get the data of nifty 50 stock.
- For this what are the steps i need to perform to get this working.
## 7. For that stock get the all the relations/updates/factors internally and globally which might effect the price of that stock
## 8. monthly efffects, short term and long term effects.
## 9. Try to get the orderflow for that stock with market profile.
## 10. Design the setup based on all the above factors with stratergy - daily, weekly, monthly.
## 11. Make trade decision autonomous.
## 12. Create order ,execute based on the APIs of the broker. - bot which can do all the trade part.
## 13. back test and forward test the strategy


